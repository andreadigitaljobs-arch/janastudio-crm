...
const BarberPanel = ({ isMobile, rates }) => {
  ...
  useEffect(() => {
    if (!selectedBarber) return undefined;
    let refreshTimer;
    ...
    try {
      ...
      const totals = transactions.reduce((summary, transaction) => {
        const member = (transaction.metadata?.staffInvolved || []).find(
          item => String(item.staffId || item.id) === String(selectedBarber.id)
        );
        ...
        const itemCount = isAssistant
          ? Math.max(Number(transaction.metadata?.washCount || 0), 1)
          : Math.max(transaction.metadata?.appointmentIds?.length || (transaction.metadata?.appointment_id ? 1 : 0), 1);
        return {
          production: summary.production + (isAssistant ? commission + productCommission + tips : Number(transaction.amount || 0)),
  ...
  const handleAssistantStartWash = async (appId) => {
    try {
      setLoading(true);
      const app = myServices.find(s => s.id === appId);
      const clientName = app?.clients?.name || 'Cliente';
      await dataService.updateAppointmentStatus(appId, 'En Lavado');
      showToast(`¡Lavado iniciado para ${clientName}!`, 'success');
      // Notify barber
      notificationService.broadcastNotification(
        supabase,
        '💧 Lavado Iniciado',
        `El asistente ha iniciado el lavado de ${clientName}.`,
        { recipientRole: 'Admin' }
      );
      loadMyWork();
      loadStats();
      ...
      console.error(err);
      showToast('Error al iniciar lavado', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: isMobile ? '580px' : '1100px', margin: '0 auto', paddingBottom: '100px' }}>
      ...
      <div style={{ display: isMobile ? 'block' : 'grid', gridTemplateColumns: '3fr 2fr', gap: '28px', alignItems: 'start' }}>
        ...
        {isAssistant ? (
                          ...
                          <div key={ex.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.03)', padding: '10px 14px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                              {editingExtraPriceId === ex.id ? (
                                ...
                                <div style={{ textAlign: 'right' }}>
                                  <span style={{ color: 'var(--gold-primary)', fontWeight: '800', display: 'block' }}>
              ...
              {myServices.filter(app => 
                ...
                myServices.filter(app => 
                  ...
                  <div key={app.id} className="glass-card animate-slide-up" style={{ borderRadius: '20px', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', opacity: 0.85, background: 'rgba(255,255,255,0.02)' }}>
                    ...
                    <button
                      onClick={() => handleAssistantStartWash(app.id)}
                      disabled={loading}
                      style={{ 
                        fontSize: '11px', 
                        fontWeight: '900', 
                    ...
                    >
                      <Droplets size={14} /> INICIAR LAVADO
                    </button>
                  </div>
                ))
              )}
          ...
          <section>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {myServices.length === 0 ? (
                <div className="glass-card" style={{ textAlign: 'center', padding: isMobile ? '60px' : '36px 40px', borderRadius: '24px', border: '2px dashed rgba(212,175,55,0.2)', background: 'rgba(212,175,55,0.02)' }}>
                  <style>{`
                        ...
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                            <span style={{ 
                              fontSize: '9px', 
                              fontWeight: '900', 
                              color: app.status === 'En Lavado' ? '#007aff' : app.status === 'Agendado' ? '#ff9f0a' : 'var(--gold-primary)', 
                              textTransform: 'uppercase', 
                              letterSpacing: '1.5px',
                              background: app.status === 'En Lavado' ? 'rgba(0,122,255,0.12)' : app.status === 'Agendado' ? 'rgba(255,159,10,0.12)' : 'rgba(212,175,55,0.1)',
                              padding: '4px 10px',
                              borderRadius: '8px',
                              border: app.status === 'En Lavado' ? '1px solid rgba(0,122,255,0.2)' : app.status === 'Agendado' ? '1px solid rgba(255,159,10,0.25)' : '1px solid rgba(212,175,55,0.15)',
                              whiteSpace: 'nowrap',
                              display: 'inline-flex',
                              alignItems: 'center',
                              gap: '6px'
                              ...
                              {app.status} 
                              {app.status === 'En Lavado' ? '💧' : app.status === 'Agendado' ? '📅' : (
                                <img 
                                  src="/barber-chair.png" 
                                  alt="Silla" 
                                  style={{ 
        ...
        <section style={{
          display: 'grid',
          gridTemplateColumns: isAssistant ? 'repeat(2, 1fr)' : 'repeat(2, 1fr)', 
          gap: '16px' 
        }}>
          <div className="glass-card" style={{ padding: isMobile ? '20px 16px' : '28px 20px', borderRadius: '24px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '8px' }}>
              <div style={{ fontSize: '24px', fontWeight: '900', color: isAssistant ? '#007aff' : 'var(--gold-primary)', textShadow: isAssistant ? '0 0 15px rgba(0,122,255,0.4)' : '0 0 15px rgba(212,175,55,0.4)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '18px', fontWeight: '900', color: isAssistant ? '#007aff' : '#32d74b', textShadow: isAssistant ? '0 0 10px rgba(0,122,255,0.4)' : '0 0 10px rgba(50,215,75,0.4)' }}>
                          {Math.round(displayVal * (rates?.usd || 550)).toLocaleString('es-VE')} Bs. {isAssistant && <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>(Comisión)</span>}
                        </div>
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: '800', marginTop: '2px' }}>
              ...
              <div style={{ padding: '20px 24px' }}>
                ...
                <div style={{ 
                  ...
                  <div style={{ borderTop: isAssistant ? '1px solid rgba(0,122,255,0.15)' : '1px solid rgba(50,215,75,0.15)', paddingTop: '12px', marginTop: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '22px', fontWeight: '950', color: isAssistant ? '#007aff' : '#32d74b' }}>
